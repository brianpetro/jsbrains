...
const SOURCE_CONNECTIONS_CACHE = {};
...
async function find_connections(params={}) {
  ...
  if(!exclude_blocks_from_source_connections) {
    ...
    if(!SOURCE_CONNECTIONS_CACHE[cache_key]){
      const nearest = (await this.env.smart_blocks.entities_vector_adapter.nearest(this.vec, filter_opts))
        .sort(sort_by_score)
        .slice(0, limit)
      ;
      connections_to_cache(cache_key, nearest);
    }
    connections = [
      ...connections,
