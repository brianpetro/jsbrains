 ...
 */
class FsAdapter extends SmartCollectionsAdapter {
  /**
   * Creates an instance of FsAdapter.
   * @param {Object} main - The main instance of the smart-collections-2 plugin.
   */
  constructor(main) {
    super(main);
  }
  async mkdir(path) { return await fs.mkdir(path, { recursive: true }); }
  async read(path) { return await fs.readFile(path, 'utf8'); }
  async write(path, data) { return await fs.writeFile(path, data); }
  async list(path) { 
    const entries = await fs.readdir(path, { withFileTypes: true });
    return {
      files: entries.filter(entry => entry.isFile()).map(file => `${path}/${file.name}`)
    };
  }
  async exists(path) { 
    try {
  ...
  async stat(path) { return await fs.stat(path); }
  async append(path, data) { return await fs.appendFile(path, data); }
  async remove(path) { return await fs.unlink(path); }
  /**
