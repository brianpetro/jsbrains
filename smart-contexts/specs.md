...
2. Iterates over each `depth`, building "chunks." For each item:
		...
		```
		When the top-level template contains `{{FILE_TREE}}`, an ASCII folder tree
		is generated via `build_file_tree_string(all_paths)` from `smart-utils` using
		the collected `path` values.
