...
test.serial('SmartSource move_to operation - new file', async t => {
    ...
    await env.fs.write('source.md', 'Initial content');
...
test.serial('SmartSource move_to operation - existing file (merge)', async t => {
    const env = t.context.env;
    env.smart_change = null;
    await env.fs.write('from.md', 'from content');
    const source = await env.smart_sources.create_or_update({ path: 'from.md' });
    await env.fs.write('to.md', 'to content');
...
test.serial('SmartBlock move_to operation - move to a new file', async t => {
  const env = t.context.env;
  env.smart_change = null;
  await env.fs.write('source.md', '# Header 1\nContent 1\n# Header 2\nContent 2');
  const source = await env.smart_sources.create_or_update({ path: 'source.md' });
...
test.serial('SmartBlock move_to operation - move to an existing file', async t => {
  ...
  await env.fs.write('existing.md', 'Existing content');
...
test.serial('SmartBlock move_to operation - move within the same file', async t => {
  const env = t.context.env;
  env.smart_change = null;
  await env.fs.write('multi_block.md', '# Header 1\nContent 1\n# Header 2\nContent 2\n# Header 3\nContent 3');
...
test.serial('SmartBlock move_to operation - move within the same file with sub-blocks', async t => {
  const env = t.context.env;
  env.smart_change = null;
  await env.fs.write('multi_block_with_subblocks.md', '# Header 1\nContent 1\n## Subheader\nSubcontent\n# Header 2\nContent 2\n# Header 3\nContent 3');
...
test.serial('SmartBlock move_to operation - preserve sub-block keys when moving', async t => {
  const env = t.context.env;
  env.smart_change = null;
  await env.fs.write('source_with_subblocks.md', '# Header 1\nContent 1\n## Subheader 1\nSubcontent 1\n## Subheader 2\nSubcontent 2\n# Header 2\nContent 2');
  ...
  t.true(await env.fs.exists('new_file.md'), 'New file should be created');
  t.is(
    await env.fs.read('new_file.md'),
    '# Header 1\nContent 1',
    'Moved block content should include subheaders in the new file'
  );
  t.is(
    (await env.fs.read('source_with_subblocks.md')).trim(),
    '# Header 1\n## Subheader 1\nSubcontent 1\n## Subheader 2\nSubcontent 2\n# Header 2\nContent 2',
...
test.serial('SmartSource move_to operation with nested paths', async t => {
  const env = t.context.env;
  await env.fs.write('folder1/source.md', 'Nested content');
...
test.serial('SmartSource move_to operation - existing file (merge with replace_all)', async t => {
    const env = t.context.env;
    env.smart_change = null;
    await env.fs.write('from.md', '# Header 1\nContent 1\n# Header 2\nContent 2');
    const source = await env.smart_sources.create_or_update({ path: 'from.md' });
    await env.fs.write('to.md', '# Existing Header\nExisting content');
    const target = await env.smart_sources.create_or_update({ path: 'to.md' });
...
test.serial('SmartBlock move_to operation - move to specific position in existing file', async t => {
    ...
    await env.fs.write('target.md', '# Existing Header\nExisting content\n# Another Header\nMore content');
...
test.serial('SmartSource move_to operation with smart_change syntax', async t => {
  ...
  await env.fs.write('source.md', initial_content);
  const source = await env.smart_sources.create_or_update({ path: 'source.md' });
  t.false(await env.fs.exists('new_location.md'), 'New file should not exist before moving');
  await source.move_to('new_location.md');
  t.true(await env.fs.exists('source.md'), 'Old file should exist with smart_change syntax');
  t.true(await env.fs.exists('new_location.md'), 'New file should exist');
  ...
  t.is(await env.fs.read('new_location.md'), expected_content_new_location, 'Content should include smart_change syntax');
...
>>>>>>> MOVED_TO
${initial_content}`;
  t.is(await env.fs.read('source.md'), expected_content_source, 'Content should include smart_change syntax');
});
...
test.serial('SmartBlock move_to operation with smart_change syntax', async t => {
  ...
  ${block_1}
  >>>>>>>`.split("\n").map(line => line.trim()).join("\n");
  t.is(await env.fs.read('new_file.md'), expected_new_content, 'Moved block content should include smart_change syntax');
  ...
  >>>>>>> MOVED_TO
  ${block_2}`.split("\n").map(line => line.trim()).join("\n");
  t.is(await env.fs.read('source.md'), expected_source_content, 'Original file should contain smart_change syntax');
});
