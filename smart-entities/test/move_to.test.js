...
test.serial('SmartBlock move_to operation - preserve sub-block keys when moving', async t => {
  const env = t.context.mock_env;
  await env.smart_fs.write('source_with_subblocks.md', '# Header 1\nContent 1\n## Subheader 1\nSubcontent 1\n## Subheader 2\nSubcontent 2\n# Header 2\nContent 2');
  ...
  t.true(await env.smart_fs.exists('new_file.md'), 'New file should be created');
  t.is(
    await env.smart_fs.read('new_file.md'),
    '# Header 1\nContent 1',
    'Moved block content should include subheaders in the new file'
  );
  t.is(
    (await env.smart_fs.read('source_with_subblocks.md')).trim(),
    '# Header 1\n## Subheader 1\nSubcontent 1\n## Subheader 2\nSubcontent 2\n# Header 2\nContent 2',
...
test.serial('SmartSource move_to operation with smart_change syntax', async t => {
  const env = t.context.mock_env;
  env.smart_change = new SmartChange(t.context.mock_env, {
    adapters: {
      default: new DefaultAdapter()
    }
  });
  const initial_content = 'Initial content';
  await env.smart_fs.write('source.md', initial_content);
  const source = await env.smart_sources.create_or_update({ path: 'source.md' });
  t.false(await env.smart_fs.exists('new_location.md'), 'New file should not exist before moving');
  await source.move_to('new_location.md');
  t.true(await env.smart_fs.exists('source.md'), 'Old file should exist with smart_change syntax');
  t.true(await env.smart_fs.exists('new_location.md'), 'New file should exist');
  ...
  t.is(await env.smart_fs.read('new_location.md'), expected_content_new_location, 'Content should include smart_change syntax');
...
>>>>>>> MOVED_TO
${initial_content}`;
  t.is(await env.smart_fs.read('source.md'), expected_content_source, 'Content should include smart_change syntax');
});
...
test.serial('SmartBlock move_to operation with smart_change syntax', async t => {
  ...
  ${block_1}
  >>>>>>>`.split("\n").map(line => line.trim()).join("\n");
  t.is(await env.smart_fs.read('new_file.md'), expected_new_content, 'Moved block content should include smart_change syntax');
